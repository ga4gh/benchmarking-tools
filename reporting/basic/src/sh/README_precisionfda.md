...
* **Custom stratification regions**: it is possible to supply a tar.gz file which contains a custom set of stratification regions. This file must contain the bed files and exactly one tsv file (in the root directory) which gives names and relative paths to these files (see [https://github.com/ga4gh/benchmarking-tools/blob/master/resources/stratification-bed-files/ga4gh_other.tsv](https://github.com/ga4gh/benchmarking-tools/blob/master/resources/stratification-bed-files/ga4gh_other.tsv) for an example).
* **Adjust confident regions**: when using partial credit preprocessing, variants might move outside the confident regions.
  ...
  option since there are some corner cases in which they may get added back in this way.

# Why does it fail on my VCF?

This app requires its input VCF to have a complete header which contains correct types for all INFO and FORMAT
fields, as well as contig definitions for all contigs in the file. This is a requirement from htslib when translating
VCF files into BCF format (which is the internal format that hap.py uses for most comparison modes).
